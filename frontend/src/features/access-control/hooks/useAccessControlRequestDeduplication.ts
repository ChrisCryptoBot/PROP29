/**
 * Access Control Request Deduplication Hook
 * Prevents duplicate API calls during rapid user interaction or WebSocket updates
...
export function useAccessControlRequestDeduplication() {
  ...
  const isDuplicate = useCallback((requestKey: string): boolean => {
    ...
    logger.debug('Duplicate request detected, skipping', {
      module: 'AccessControlRequestDeduplication',
      requestKey,
  ...
  const recordRequest = useCallback((requestKey: string, ...args: unknown[]) => {
