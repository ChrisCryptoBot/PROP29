...
interface CreateUserModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (userData: Partial<AccessControlUser>) => Promise<void>;
  isFormDirty: boolean;
...
export const CreateUserModal: React.FC<CreateUserModalProps> = ({
  ...
  return (
    <Modal
      ...
      footer={
        <>
          <Button onClick={handleClose} variant="subtle" disabled={isSubmitting} className="text-xs font-black uppercase tracking-widest">
            Cancel
          </Button>
          <Button onClick={handleSubmit} variant="primary" disabled={isSubmitting || !isFormDirty} aria-label="Create user" className="text-xs font-black uppercase tracking-widest shadow-none">
    ...
    >
      <div className="space-y-8">
        ...
        <div className="space-y-4">
          ...
          <div>
            <label htmlFor="user-email" className="block text-xs font-bold text-white mb-2 uppercase tracking-wider">Email <span className="text-red-400">*</span></label>
            <input
              type="email"
              id="user-email"
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label htmlFor="user-department" className="block text-xs font-bold text-white mb-2 uppercase tracking-wider">Department <span className="text-red-400">*</span></label>
              <input
                type="text"
                id="user-department"
            ...
            <div>
              <label htmlFor="user-phone" className="block text-xs font-bold text-white mb-2 uppercase tracking-wider">Phone</label>
              <input
                type="tel"
                id="user-phone"
          ...
          <div>
            <label htmlFor="user-employee-id" className="block text-xs font-bold text-white mb-2 uppercase tracking-wider">Employee ID</label>
            <input
              type="text"
              id="user-employee-id"
        ...
        <div className="space-y-4">
            ...
            <div className="grid grid-cols-7 gap-2">
              {WEEKDAYS.map(day => (
                <label
                  key={day}
                  className={cn(
                    'flex flex-col items-center justify-center p-2 border rounded-md cursor-pointer transition-colors',
                    formData.accessSchedule?.days.includes(day)
          ...
          {formData.accessSchedule && formData.accessSchedule.days.length > 0 && (
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label htmlFor="schedule-start-time" className="block text-xs font-bold text-white mb-2 uppercase tracking-wider">Start Time</label>
                <input
                  type="time"
              ...
              <div>
                <label htmlFor="schedule-end-time" className="block text-xs font-bold text-white mb-2 uppercase tracking-wider">End Time</label>
                <input
                  type="time"
