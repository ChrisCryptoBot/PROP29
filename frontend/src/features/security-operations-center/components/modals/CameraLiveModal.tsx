...
import { controlCameraPTZ, sendCameraHeartbeat } from '../../services/securityOperationsCenterService';
...
export const CameraLiveModal: React.FC<CameraLiveModalProps> = ({ modalId, camera }) => {
  ...
  const { refreshCameras } = useSecurityOperationsContext();
  ...
  // When opening live view for a camera with a stream, send heartbeat so backend marks it online; then refresh so badge shows "Online"
  useEffect(() => {
    if (heartbeatSentRef.current || !camera.streamUrl) return;
    heartbeatSentRef.current = true;
    (async () => {
      const ok = await sendCameraHeartbeat(camera.id);
      if (ok) await refreshCameras();
    })();
  }, [camera.id, camera.streamUrl, refreshCameras]);
  ...
  const handleSnapshot = useCallback((videoElement?: HTMLVideoElement) => {
    const video = videoElement ?? (panelRef.current?.querySelector('video') as HTMLVideoElement);
  ...
  const modalContent = (
    ...
    >
      <div
        ref={panelRef}
        className={cn(
          'absolute bg-slate-900 rounded-md border border-white/5 overflow-hidden flex flex-col pointer-events-auto'
        )}
        style={panelStyle}
      ...
      >
        ...
        {/* Video area — show live feed when we have a stream URL (even if backend still says offline); maintenance/truly no stream show placeholders */}
        <div className="flex-1 min-h-0 relative bg-black/40 p-4">
          {camera.status === 'maintenance' && (
            <div className="w-full min-h-[240px] rounded-md flex flex-col items-center justify-center gap-3 bg-slate-900/90 border border-amber-500/20">
              <i className="fas fa-wrench text-4xl text-amber-400/80" aria-hidden />
              <span className="text-sm font-black uppercase tracking-widest text-amber-400">Maintenance</span>
              <span className="text-xs text-slate-500">No live feed</span>
            </div>
          )}
          {camera.status !== 'maintenance' && camera.streamUrl && (
            <div className="relative w-full h-full min-h-[240px] rounded-md overflow-hidden">
              <VideoStreamPlayer
                src={camera.streamUrl}
                poster={camera.lastKnownImageUrl}
                className="w-full h-full rounded-md bg-slate-900 object-contain"
                errorClassName="w-full h-full rounded-md bg-slate-900 flex items-center justify-center text-slate-400 text-sm border border-white/5"
                showTimestamp={true}
                timestampPosition="top-left"
                onScreenshot={handleSnapshot}
                cameraId={camera.id}
              />

              {/* PTZ Controls - Standard 3x3 grid layout */}
              <div className="absolute top-4 right-4 bg-black/60  rounded-lg p-3" role="group" aria-label="Camera pan, tilt, and zoom controls">
                ...
                {/* Zoom Controls — slider is visual only until backend supports level feedback */}
                <div className="flex items-center gap-1 mt-3" role="group" aria-label="Zoom controls">
                  ...
                  <div className="flex-1 h-1 bg-white/20 rounded mx-2 relative flex items-center justify-center" aria-hidden="true">
                    <div className="absolute left-1/2 w-3 h-3 bg-white rounded-full -mt-1 transform -translate-x-1/2" />
                    <span className="relative text-[9px] text-white/50 uppercase tracking-wider">Level</span>
                  </div>
              ...
              <div className="absolute top-16 left-4 flex flex-col gap-2">
                <button className="bg-black/60  rounded-lg px-3 py-2 flex items-center gap-2 hover:bg-black/80 transition-colors">
          ...
          {camera.status !== 'maintenance' && !camera.streamUrl && camera.status === 'offline' && (
            <div className="w-full min-h-[240px] rounded-md flex flex-col items-center justify-center gap-3 bg-slate-900/90 border border-rose-500/20">
              <i className="fas fa-video-slash text-4xl text-rose-400/80" aria-hidden />
              <span className="text-sm font-black uppercase tracking-widest text-rose-400">Offline</span>
              <span className="text-xs text-slate-500">No live feed. Recorded footage is stored separately.</span>
            </div>
          )}
          {camera.status !== 'maintenance' && !camera.streamUrl && camera.status !== 'offline' && (
            <div className="w-full min-h-[240px] rounded-md bg-slate-900 flex items-center justify-center text-slate-400 border border-white/5">
